...
public:
  SCurveMotionController(float min_speed, float max_speed, float max_accel,
                         float max_jerk, float short_dist)
      : min_speed_(min_speed), max_speed_(max_speed), max_accel_(max_accel),
        max_jerk_(max_jerk), short_dist_(short_dist), speed_(0), accel_(0),
        peak_speed_(0) {}
  ...
  float min_speed_, max_speed_, max_accel_, max_jerk_, short_dist_;
