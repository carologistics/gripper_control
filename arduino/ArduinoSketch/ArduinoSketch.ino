 ...
 * It is thus important, that the low byte is read before the high byte, so the
 * shadow register is properly loaded.
 */
...
void double_calibrate() {
  ...
  float speeds[3] = {motor_X.get_speed(), motor_Y.get_speed(),
                     motor_Z.get_speed()};
...
void calibrate() {
  ...
  do { // repeat calibration as long as not successfull
    ...
    set_status(
        STATUS_MOVING); // status is always only changed on no interrupt code
                        // level, hence no race condition occurs here
    // This while loop controls permanently the state of the respective end
    // stops and handles crashing into them When all end stops are triggered
    // simulatenously, additional latency is introduced. The latency is maily
    // due to the planning of the back movement. One work around is to calibrate
    // twice, the first time fast and the second time slowly. (again inspired
    // from grbl) Additionally, the backwards movement should use different
    // numbers of steps to ensure that the end stops will not be triggerend
    // simulatenously at the second calibration.
...
void set_new_pos(long new_pos, AccelStepper &motor) {
  ...
  motor_Y.enableOutputs();
  motor_Z.enableOutputs();
  noInterrupts(); // shortly disable interrupts to preverent stepping while
                  // changing target position (this is actually only a problem
                  // when cur_status == STATUS_MOVING)
  motor.moveTo(new_pos);
  interrupts();              // activate interrupts again
  set_status(STATUS_MOVING); // status is always only changed on no interrupt
                             // code level, hence no race condition occurs here
}
...
inline void set_new_speed_acc(float new_speed, float new_acc,
                              AccelStepper &motor) {
...
// stop all motors with infinite acceleration. Do not use if step counter should
...
void read_package() {
  ...
  while (true) {
    ...
    if (next_char == TERMINATOR) {
      ...
      buffer_[buf_i_] = 0;
      // Set null character to get no trouble with sscanf // buf_i_ points now
      // onto 0
      break;
    } else if (next_char == -1) {
      // if no serial data is available anymore, but package terminator not
      // found yet:
      return; // cannot do anything now
    }
    buffer_[buf_i_++] = next_char; // other characters are added to the buffer
    if (buf_i_ >= BUFFER_SIZE) {
      // Buffer overflow. Strategy: flush buffer and start new. (This should not
      // happen normally)
      buf_i_ = 0;
      return;
    }
  ...
  for (package_start = 0; package_start < buf_i_ - 2; package_start++) {
    if (buffer_[package_start] == 'A' && buffer_[package_start + 1] == 'T' &&
      ...
      break; // explicitly break, otherwise package_start++ is executed once
             // again
    }
  }

  if (!package_located) { // terminator symbol was reached but no package was
                          // located
  ...
  while (cur_i_cmd < buf_i_) {
    ...
    // this is used to store the assumed gripper state locally, to reduce calls
    // to the function get_assumed_gripper_state
    switch (cur_cmd) {
    case CMD_X_NEW_POS:
...
void setup() {
  ...
  TIMSK0 =
      0; // turn off interrupts on Timer 0. micros() will thus NOT work anymore!
  /* Why use timer 0 for the step interrupt?
   *  Why use timer 2 for the pule interrupts?
   *  By this procedure it's made sure that the pulse interrupts fire BEFORE a
   * new step interrupt is called, in case both are pending
   */
  ...
  TCCR2B =
      0x0;   // no clock source, activate with TCCR2B=0x1; just direct io clock
  TCNT2 = 0; // reset counter
  OCR2A =
      3; // 15; // start pulse, should be at least 650ns after setting direction
...
ISR(TIMER0_COMPA_vect) // this is called every overflow of the timer 0
{
  ...
  if (cur_status == STATUS_MOVING) {
    ...
    if (step) { // only step if really necessary
      while (!pulse_done)
        ; // wait until the previous pulse is done // TODO:: remove after
          // ensuring pulse is surely done here EVERY TIME!
      // Serial.println(dir,BIN);
      dir ^= 1 << MOTOR_X_DIR_SHIFT;
      dir ^= 1 << MOTOR_Y_DIR_SHIFT;
      MOTOR_XYZ_DIR_OUT = (MOTOR_XYZ_DIR_OUT & MOTOR_XYZ_DIR_INV_MASK) |
                          dir; // set the direction pins right
      step_bits_xyz = step;
...
{
  MOTOR_XYZ_STEP_OUT &= ~step_bits_xyz; // stopping the pulse has higher
                                        // priority than turning off the timer
