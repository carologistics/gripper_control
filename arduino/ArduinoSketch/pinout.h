 ...
 * For this purpose, the following masks can be logical anded with the previous
 * state leaving all other pins unchanged.
 */

#define MOTOR_XYZ_DIR_INV_MASK                                                 \
  ~((1 << MOTOR_X_DIR_SHIFT) | (1 << MOTOR_Y_DIR_SHIFT) |                      \
    (1 << MOTOR_Z_DIR_SHIFT))
