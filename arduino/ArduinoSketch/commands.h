...
class ArduinoHelper {
public:
  static bool isValidSerialCommand(char cmd) {
    if (cmd == CMD_CALIBRATE || cmd == CMD_DOUBLE_CALIBRATE ||
        cmd == CMD_X_NEW_POS || cmd == CMD_Y_NEW_POS || cmd == CMD_Z_NEW_POS ||
        cmd == CMD_CLOSE || cmd == CMD_OPEN || cmd == CMD_STOP ||
        cmd == CMD_FAST_STOP || cmd == CMD_STATUS_REQ ||
        cmd == CMD_A_SET_TOGGLE_STEPS || cmd == CMD_X_NEW_SPEED ||
        cmd == CMD_Y_NEW_SPEED || cmd == CMD_Z_NEW_SPEED ||
        cmd == CMD_A_NEW_SPEED || cmd == CMD_SET_SPEED ||
